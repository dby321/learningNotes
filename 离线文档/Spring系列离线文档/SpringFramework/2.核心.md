...
> ```
>
> 这表明查找方法注入已经成功地通过`CommandManager`创建了`AsyncCommand`实例，并执行了命令逻辑。
...
> ### 1. 创建目标类 (`MyValueCalculator`)
>
> 首先，我们定义一个简单的类`MyValueCalculator`，它有一个需要被替换的方法`computeValue(String)`。
>
> java
>
> 深色版本
...
> ```
>
> ### 2. 创建替换逻辑类 (`ReplacementComputeValue`)
>
> 接下来，创建一个实现了`org.springframework.aop.framework.MethodReplacer`接口的类`ReplacementComputeValue`，用于提供新的方法实现。
>
> java
>
> 深色版本
...
> package com.example;
> 
> import org.springframework.aop.framework.MethodReplacer;
...
> ### 3. Spring配置
>
> 然后，在Spring XML配置文件中定义bean，并指定要替换的方法。
>
> xml
>
> 深色版本
...
>         ApplicationContext context = new ClassPathXmlApplicationContext("applicationContext.xml");
>         
>         MyValueCalculator calculator = (MyValueCalculator) context.getBean("myValueCalculator");
>         
>         // 调用原本应该被替换的方法
>         System.out.println(calculator.computeValue("hello"));  // 应该输出 "Replaced: HELLO"
>     }
> }
> ```
>
...
> ```
>
> 这表明`computeValue`方法已经被`ReplacementComputeValue`提供的新逻辑所替换。
...
> ### 为什么需要`<aop:scoped-proxy/>`
>
> 在Spring中，默认情况下所有bean都是单例模式，即在整个应用上下文中只有一个实例。然而，当你尝试将一个非单例作用域的bean（如`session`或`request`作用域）注入到一个单例bean中时，**会出现问题，因为单例bean在其生命周期内只会持有最初注入的那个非单例bean的一个实例引用，这显然不是我们想要的行为**。
>
> 通过使用`<aop:scoped-proxy/>`，Spring会为`userPreferences`创建一个CGLIB或JDK动态代理（取决于类是否可以继承），这个代理可以在每次访问`userPreferences`时返回正确的、对应当前作用域（例如当前HTTP会话）的实例。因此，尽管`userManager`是一个单例，但它每次调用`userPreferences`时都会得到当前会话对应的最新`userPreferences`实例，而不是最初注入的那个旧实例。
...
> ### 1. 创建自定义作用域类
>
> 首先，我们需要创建一个实现了`org.springframework.beans.factory.config.Scope`接口的类。这个类负责管理该作用域内的bean实例。
>
> java
>
> 深色版本
...
> 
>     private final ThreadLocal<Map<String, Object>> scope = new ThreadLocal<>();
> 
>     @Override
>     public Object get(String name, ObjectFactory<?> objectFactory) {
>         Map<String, Object> scopedBeans = this.scope.get();
>         if (scopedBeans == null) {
>             scopedBeans = new HashMap<>();
>             this.scope.set(scopedBeans);
>         }
> 
>         if (!scopedBeans.containsKey(name)) {
>             scopedBeans.put(name, objectFactory.getObject());
>         }
> 
>         return scopedBeans.get(name);
>     }
> 
>     @Override
...
>         // 在这个简单的示例中，我们不实现上下文对象解析
>         return null;
>     }
> 
>     @Override
>     public String getConversationId() {
>         // 返回一个标识符，用于区分不同的会话或线程等
...
> ### 2. 注册自定义作用域
>
> 接下来，我们需要在Spring容器中注册这个自定义作用域。这可以通过实现`BeanFactoryPostProcessor`接口并在其`postProcessBeanFactory`方法中调用`registerScope`方法来完成。
>
> java
>
> 深色版本
...
> ### 3. 使用自定义作用域
>
> 现在，我们可以在配置文件或注解中使用新定义的作用域。这里以XML配置为例：
>
> xml
>
> 深色版本
...
> 
>     <!-- 定义一个使用自定义作用域的bean -->
>     <bean id="myScopedBean" class="fiona.apple.MyScopedBean" scope="custom">
>         <!-- 配置依赖项和其他属性 -->
>     </bean>
> 
> </beans>
...
> 
>         System.out.println("Same instance? " + (bean1 == bean2));  // 应该输出 false
> 
>         // 切换线程并再次获取bean实例
>         Thread thread = new Thread(() -> {
>             MyScopedBean beanInThread = context.getBean(MyScopedBean.class);
>             System.out.println("Different thread, same instance as bean1? " + (beanInThread == bean1));  // 应该输出 false
>         });
> 
>         thread.start();
>     }
...
> ### 5. 自定义作用域中的Bean
>
> 为了完整起见，这里提供了一个简单的`MyScopedBean`类作为示例。
>
> java
>
> 深色版本
...
> ```
> Same instance? false
> Different thread, same instance as bean1? false
> ```
>
> 这表明自定义作用域已经成功地为每个线程维护了独立的bean实例。
...
> ### 1. 创建实现 `SmartLifecycle` 接口的类
>
> 首先，我们创建一个名为`MySmartLifecycleBean`的类，它实现了`SmartLifecycle`接口。在这个类中，我们将实现所有必要的方法，并添加一些日志输出以便于观察其行为。
>
> java
>
> 深色版本
...
>     @Override
>     public int getPhase() {
>         // 设置phase值为最小，确保它是第一个启动和最后一个停止的bean
...
> ### 2. 配置生命周期处理器（可选）
>
> 如果你想要调整默认的生命周期处理器的行为，比如改变每个阶段的最大等待时间，可以在配置文件中定义一个自定义的`lifecycleProcessor` bean。这里以XML配置为例：
>
> > 每个阶段的最大等待时间（`timeoutPerShutdownPhase`）是指在Spring应用上下文关闭过程中，对于实现了`SmartLifecycle`接口的Bean，在调用其`stop(Runnable callback)`方法后，Spring框架等待该Bean完成其停止逻辑并调用提供的回调函数的最大时间。
>
> xml
>
> 深色版本
...
>         // 设置每个阶段的最大等待时间为10秒
>         processor.setTimeoutPerShutdownPhase(10000);
>         return processor;
>     }
> }
> ```
...
> ### 3. 测试代码
>
> 最后，编写一个简单的测试程序来验证`SmartLifecycle` Bean的工作情况。你可以通过启动和关闭`ApplicationContext`来触发Bean的启动和停止过程。
>
> java
>
> 深色版本
...
>
> ```
> Starting MySmartLifecycleBean
> Stopping MySmartLifecycleBean
> ```
>
> 这表明`MySmartLifecycleBean`已经在应用程序上下文刷新后自动启动，并且在上下文关闭时正确地停止了。
...
> ### 1. 创建带有销毁方法的Bean
>
> 首先，我们创建一个名为`MyResourceBean`的类，它实现了一个简单的资源管理逻辑，并指定了销毁方法。
>
> java
>
> 深色版本
...
> ### 2. 配置Spring上下文
>
> 接下来，我们需要配置Spring上下文以声明`MyResourceBean`并指定其销毁方法。这里以XML配置为例：
>
> xml
>
> 深色版本
...
>            http://www.springframework.org/schema/beans/spring-beans.xsd">
> 
>     <!-- 定义MyResourceBean并指定销毁方法 -->
>     <bean id="myResourceBean" class="fiona.apple.MyResourceBean" destroy-method="releaseResources"/>
> 
> </beans>
> ```
>
...
> public class AppConfig {
> 
>     @Bean(destroyMethod = "releaseResources")
...
> ### 3. 注册关闭钩子
>
> 现在，我们将编写主程序代码，初始化Spring上下文并注册一个关闭钩子。这将确保当JVM终止时调用所有已配置的销毁方法。
>
> java
>
> 深色版本
...
>         Thread.sleep(5000);
> 
>         // 正常情况下，main方法退出后，关闭钩子会被调用
>         // 如果你想要显式关闭上下文，也可以调用以下方法：
>         // ctx.close();
>     }
> }
> ```
...
>         System.out.println("ApplicationContext injected into " + this.beanName);
>     }
> 
>     @Override
...
> ```
>
> - **`getOrder()` 方法**：返回一个整数，表示实现类的顺序。**这个值越小，优先级越高；反之，值越大，优先级越低。**
...
> ### 1. 创建自定义 `FactoryBean`
>
> 首先，我们创建一个名为`MyComplexObjectFactoryBean`的类，它实现了`FactoryBean<MyComplexObject>`接口。我们将在这个类中编写复杂的初始化逻辑。
>
> java
>
> 深色版本
...
> import org.springframework.stereotype.Component;
> 
> @Component("myComplexObjectFactoryBean")
...
>             complexObject.initialize(); // 假设有一个initialize方法进行初始化
>         }
...
>         // 如果返回true，则每次调用getObject()都会返回同一个实例
>         // 如果返回false，则每次调用getObject()都会返回一个新的实例
>         return true;
>     }
> }
> ```
...
> ### 2. 定义复杂对象
>
> 接下来，我们定义一个名为`MyComplexObject`的类，模拟一个需要复杂初始化的对象。
>
> java
>
> 深色版本
...
>         // 复杂的初始化逻辑
>         System.out.println("Initializing MyComplexObject...");
>     }
> 
>     public void doSomething() {
>         // 业务逻辑
>         System.out.println("Doing something with MyComplexObject...");
>     }
> }
> ```
>
> ### 3. 使用 `FactoryBean`
>
> 现在，我们可以使用Spring容器来获取由`MyComplexObjectFactoryBean`生产的`MyComplexObject`实例。如果你想要获取`FactoryBean`本身而不是它产生的Bean，可以在`getBean()`方法中Bean的`id`前加上`&`符号。
>
> java
>
> 深色版本
...
> 
>         // 获取由FactoryBean生产的MyComplexObject实例
>         MyComplexObject complexObject = ctx.getBean("myComplexObjectFactoryBean", MyComplexObject.class);
>         complexObject.doSomething();
> 
>         // 如果你需要获取FactoryBean本身
>         MyComplexObjectFactoryBean factoryBean = ctx.getBean("&myComplexObjectFactoryBean", MyComplexObjectFactoryBean.class);
>         System.out.println("FactoryBean instance: " + factoryBean);
>     }
> }
> ```
>
> ### 4. 配置Spring上下文
>
> 为了使上述代码工作，我们需要确保Spring能够扫描到并注册这些组件。这里以Java配置为例：
>
> java
>
> 深色版本
...
> 
> @Configuration
> @ComponentScan(basePackages = "fiona.apple")
> public class AppConfig {
> }
> ```
>
...
>
> ```
> Initializing MyComplexObject...
> Doing something with MyComplexObject...
> FactoryBean instance: fiona.apple.MyComplexObjectFactoryBean@<hashcode>
> ```
...
>    - **作用**：解析带有`@Configuration`注解的类，并处理其中定义的`@Bean`方法。
>    - **描述**：这个后处理器对于基于Java配置的Spring应用程序非常重要，因为它负责解析和处理所有的配置类，确保它们能够按照预期工作。
> 2. **`AutowiredAnnotationBeanPostProcessor`**
>    - **作用**：处理`@Autowired`注解，包括字段、构造函数和设值方法上的使用。
>    - **描述**：这是最常用的后处理器之一，它使得开发者可以通过简单的注解来实现依赖注入，而无需显式的在XML中定义Bean之间的关系。
> 3. **`CommonAnnotationBeanPostProcessor`**
>    - **作用**：支持JSR-250标准中的注解，如`@Resource`、`@PostConstruct`和`@PreDestroy`。
>    - **描述**：通过这个后处理器，你可以利用Java EE提供的标准注解来进行资源注入和生命周期回调管理。
> 4. **`PersistenceAnnotationBeanPostProcessor`**
>    - **作用**：处理`@PersistenceUnit`和`@PersistenceContext`注解，主要用于与JPA（Java Persistence API）一起工作。
>    - **描述**：如果你的应用程序使用了JPA进行持久化操作，那么这个后处理器将帮助你更方便地管理和注入实体管理器工厂或实体管理器实例。
> 5. **`EventListenerMethodProcessor`**
>    - **作用**：处理`@EventListener`注解的方法，允许开发者定义响应特定事件的方法。
>    - **描述**：这个后处理器使得应用程序可以更容易地实现事件驱动架构，通过简单地在方法上添加`@EventListener`注解即可订阅并处理感兴趣的事件。
...
> 为了避免混淆和潜在的问题，通常建议在根应用上下文中使用`<context:annotation-config/>`或更好的选择是使用`<context:component-scan>`，它不仅会注册必要的后处理器以支持注解，还会自动扫描指定包下的类，并根据注解注册Bean。这样做可以确保整个应用程序的一致性和可维护性。
>
> ### `@ComponentScan` 和 `@Configuration` 类
>
> 现代Spring应用程序倾向于使用基于Java的配置而不是XML配置。在这种情况下，你可以在你的主配置类上使用`@ComponentScan`注解来替代XML中的`<context:component-scan>`标签。例如：
>
> java
>
> 深色版本
...
> ```
>
> 这行代码会自动扫描`com.example`包及其子包下的所有带有`@Component`、`@Service`、`@Repository`等注解的类，并注册它们为Spring容器中的Bean，同时也会激活注解驱动配置。
...
> 
>     @Autowired
>     public MovieService(MovieCatalog movieCatalog) {
>         this.movieCatalog = movieCatalog;
>     }
> 
>     // 其他业务逻辑...
...
> ```
>
> 在这个例子中，`MovieService`将自动获得`firstMovieCatalog`，因为它是`@Primary`的。
>
> 如果你想让`MovieService`使用`secondMovieCatalog`，你可以这样做：
>
> java
>
> 深色版本
...
> 
>     @Autowired
>     public MovieService(@Qualifier("secondMovieCatalog") MovieCatalog movieCatalog) {
>         this.movieCatalog = movieCatalog;
>     }
> 
>     // 其他业务逻辑...
...
> 假设你正在开发一个应用程序，它既支持在线搜索电影目录，也支持从本地缓存中检索数据以供离线使用。你可以按照以下步骤操作：
>
> 1. **定义两个不同的`MovieCatalog`实现**：
>
>    java
>
>    深色版本
...
>
>    ```java
>    @Component("onlineMovieCatalog")
>    public class OnlineMovieCatalog implements MovieCatalog {
>        // 实现在线搜索逻辑...
>    }
> 
>    @Component("offlineMovieCatalog")
>    @Offline
>    public class OfflineMovieCatalog implements MovieCatalog {
>        // 实现离线搜索逻辑...
>    }
>    ```
>
> 2. **在需要的地方使用`@Offline`注解**：
>
>    java
>
>    深色版本
...
>    public class MovieService {
> 
>        private final MovieCatalog onlineMovieCatalog;
>        private final MovieCatalog offlineMovieCatalog;
> 
>        @Autowired
>        public MovieService(@Qualifier("onlineMovieCatalog") MovieCatalog onlineMovieCatalog,
>                           @Offline MovieCatalog offlineMovieCatalog) {
>            this.onlineMovieCatalog = onlineMovieCatalog;
>            this.offlineMovieCatalog = offlineMovieCatalog;
>        }
> 
>        // 根据网络状态选择合适的MovieCatalog进行搜索...
>    }
>    ```
>
> > ### 1. 定义接口
> >
> > 首先，定义一个公共接口`MovieCatalog`，它声明了所有电影目录服务应该提供的方法。这个接口将被两个不同的实现类所共享。
> >
> > java
> >
> > 深色版本
...
> > ### 2. 创建在线搜索实现
> >
> > 接下来，创建一个实现了`MovieCatalog`接口的类`OnlineMovieCatalog`，它负责处理在线搜索逻辑。这里假设你有一个外部API来获取电影数据。
> >
> > java
> >
> > 深色版本
...
> >
> > ```java
> > @Service("onlineMovieCatalog")
> > public class OnlineMovieCatalog implements MovieCatalog {
> > 
> >     private final RestTemplate restTemplate;
> > 
> >     @Autowired
> >     public OnlineMovieCatalog(RestTemplateBuilder restTemplateBuilder) {
...
> >         // 调用外部API进行在线搜索
> >         String url = "https://api.example.com/movies?query=" + query;
> >         ResponseEntity<List<Movie>> response = restTemplate.exchange(url, HttpMethod.GET, null, new ParameterizedTypeReference<List<Movie>>() {});
...
> > ### 3. 创建离线搜索实现
> >
> > 然后，创建另一个实现了`MovieCatalog`接口的类`OfflineMovieCatalog`，它负责处理离线搜索逻辑。这里假设你有一个本地数据库或文件系统来存储电影数据。
> >
> > java
> >
> > 深色版本
...
> >
> > ```java
> > @Service("offlineMovieCatalog")
> > @Offline
> > public class OfflineMovieCatalog implements MovieCatalog {
> > 
> >     private final List<Movie> localMovies;
> > 
> >     @Autowired
> >     public OfflineMovieCatalog(@Value("classpath:offline-movies.json") Resource resource) throws IOException {
> >         // 加载本地缓存中的电影数据
> >         try (InputStream inputStream = resource.getInputStream()) {
> >             ObjectMapper objectMapper = new ObjectMapper();
> >             this.localMovies = objectMapper.readValue(inputStream, new TypeReference<List<Movie>>() {});
> >         }
> >     }
> > 
> >     @Override
...
> #### 1. 定义泛型接口
>
> 首先，定义一个泛型接口`Store<T>`，它表示可以存储任意类型的数据。
>
> java
>
> 深色版本
...
> #### 2. 创建具体实现类
>
> 接下来，创建两个实现了`Store`接口的具体类，分别用于存储字符串和整数。
>
> **Java**
>
> java
...
> #### 3. 使用泛型进行依赖注入
>
> 现在，你可以在其他地方通过接口和泛型参数来注入具体的`Store`实现。Spring将根据泛型参数自动选择正确的Bean。
>
> **Java**
>
> java
...
>     private final Store<Integer> integerStore;
> 
>     @Autowired
>     public StorageService(Store<String> stringStore, Store<Integer> integerStore) {
...
>     private val stringStore: Store<String>,
>     private val integerStore: Store<Integer>
> ) {
>     // 其他业务逻辑...
> }
> ```
...
> ### 自动注入集合类型
>
> 除了单个Bean的注入，Spring还支持自动注入集合类型的Bean（如`List<Store<Integer>>`）。这使得你可以轻松地获取所有符合特定泛型参数的Bean。
>
> **Java**
>
> java
...
> 
> // 这个列表将包含所有实现了 Store<Integer> 的 Bean
> ```
>
> **Kotlin**
>
...
> ### 示例：自动注入泛型 `List`
>
> 假设我们有一个场景，其中存在多个实现了`Store<Integer>`接口的Bean，我们希望将它们全部注入到一个服务中：
>
> **Java**
>
> java
...
> 
>     @Bean
>     public IntegerStore integerStore1() {
>         return new IntegerStore();
>     }
> 
>     @Bean
...
> public class StorageService {
> 
>     private final List<Store<Integer>> integerStores;
> 
>     @Autowired
>     public StorageService(List<Store<Integer>> integerStores) {
>         this.integerStores = integerStores;
>     }
> 
>     public void printAllIntegerStores() {
...
> 
>     @Bean
>     fun integerStore1() = IntegerStore()
> 
>     @Bean
>     fun integerStore2() = IntegerStore()
> }
> 
> @Service
> class StorageService(
>     private val integerStores: List<Store<Integer>>
> ) {
>     fun printAllIntegerStores() {
...
> ```
>
> 在这个例子中，`StorageService`中的`integerStores`列表将包含所有实现了`Store<Integer>`接口的Bean（即`integerStore1`和`integerStore2`），而不会包含任何实现了`Store<String>`接口的Bean。
