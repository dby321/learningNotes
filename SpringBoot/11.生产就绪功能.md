...
默认情况下，如果 health group 包括或排除不存在的健康指标，启动将失败。要禁用这种行为，请将 `management.endpoint.health.validate-group-membership` 设为 `false`。
...
|      | 低 cardinality 的 key-value 将被添加到指标和追踪中，而高 cardinality 的 key-value 将只被添加到追踪中。 |
| ---- | ------------------------------------------------------------ |
|      |                                                              |
...
前面的例子将阻止所有名称以 `denied.prefix` 或 `other.denied.prefix` 开头的 observation。
...
默认情况下，指标被导出到运行在你本地机器上的 [OpenTelemetry](https://micrometer.io/docs/registry/otlp) 。你可以通过以下方式提供要使用的 [OpenTelemetry metric endpoint](https://opentelemetry.io/) 的位置。
...
另外，你可以在你的环境中使用Wavefront sidecar或内部代理来转发指标数据到Wavefront API主机。
...
如果你将指标发布到Wavefront代理（如 [Wavefront文档](https://docs.wavefront.com/proxies_installing.html) 中所述），主机必须是 `proxy://HOST:PORT` 格式。
...
关于产生的观察结果（observation）的更多信息，请参见 [Spring Framework 参考文档](https://docs.spring.io/spring-framework/docs/6.1.0-M1/reference/html/integration.html#integration.observability.http-server.servlet)。
...
关于产生的观察结果（observation）的更多信息，请参见 [Spring Framework 参考文档](https://docs.spring.io/spring-framework/docs/6.1.0-M1/reference/html/integration.html#integration.observability.http-server.reactive)。
...
关于产生的观察结果（observation）的更多信息，请参见 [Spring Framework 参考文档](https://docs.spring.io/spring-framework/docs/6.1.0-M1/reference/html/integration.html#integration.observability.http-client)。
...
#### 7.3.12. Spring Batch 指标

参见 [Spring Batch 参考文档](https://docs.spring.io/spring-batch/docs/5.0.2/reference/html/monitoring-and-metrics.html)。

#### 7.3.13. Spring GraphQL 指标

参见 [Spring GraphQL 参考文档](https://docs.spring.io/spring-graphql/docs/1.2.1/reference/html/)。

#### 7.3.14. DataSource 指标
...
#### 7.3.16. Spring Data Repository 指标
...
#### 7.3.18. Spring Integration 指标

只要有 `MeterRegistry` bean，Spring Integration就会自动提供 [Micrometer support](https://docs.spring.io/spring-integration/docs/6.1.1/reference/html/system-management.html#micrometer-integration)。 度量标准在 `spring.integration.` meter 名称下发布。

#### 7.3.19. Kafka 指标

自动配置为自动配置的消费者工厂和生产者工厂分别注册了一个 `MicrometerConsumerListener` 和 `MicrometerProducerListener`。 它还为 `StreamsBuilderFactoryBean` 注册了一个 `KafkaStreamsMicrometerListener`。 更多细节，请参阅Spring Kafka文档中的 [Micrometer Native Metrics](https://docs.spring.io/spring-kafka/docs/3.0.8/reference/html/#micrometer-native) 部分。
...
自动配置为自动配置的 `LettuceConnectionFactory` 注册了一个 `MicrometerCommandLatencyRecorder`。 更多细节，请参阅Lettuce文档的 [Micrometer Metrics部分](https://lettuce.io/core/6.2.4.RELEASE/reference/index.html#command.latency.metrics.micrometer)。
...
你可以使用 [`management.observations.key-values`](https://springdoc.cn/spring-boot/actuator.html#actuator.observability.common-key-values) 属性来配置普通标签。
...
|      | 报告的测量值是所有与仪表名称相匹配的仪表和任何已应用的标签的统计量的 *总和*。 在前面的例子中，返回的 `Value` 统计是堆的 “Code Cache”、“Compressed Class Space” 和 “Metaspace” 区域的最大内存“足迹”之和。 如果你想只看到 “Metaspace” 的最大尺寸，你可以添加一个额外的 `tag=id:Metaspace` --即 `/actuator/metrics/jvm.memory.max?tag=area:nonheap&tag=id:Metaspace`。 |
| ---- | ------------------------------------------------------------ |
|      |                                                              |

### 7.7. 整合 Micrometer Observation
...
- 使用 [Zipkin](https://zipkin.io/) 、 [Wavefront](https://docs.wavefront.com/) 或 [OTLP](https://opentelemetry.io/docs/reference/specification/protocol/) 的 [OpenTelemetry](https://opentelemetry.io/)
...
- `io.micrometer:micrometer-tracing-bridge-otel` - 在 `OpenTelemetry` 中加入 Micrometer Observation API
- `io.opentelemetry:opentelemetry-exporter-zipkin` - 向 Zipkin 报告 [traces](https://micrometer.io/docs/tracing#_glossary)
...
如果你打开web浏览器访问 `localhost:8080`，你应该看到以下输出。
...
现在，在 `localhost:9411` 打开Zipkin用户界面，点击 "Run Query" 按钮，列出所有收集到的跟踪信息。你应该看到一个 trace。点击 "Show" 按钮，查看该追踪的细节。

### 8.3. 记录相关的ID

相关ID提供了一种有用的方法，可以将你的日志文件中的行与跨度/跟踪联系起来。默认情况下，只要 `management.tracing.enabled` 没有被设置为 `false`，只要你使用 Micrometer Tracing，Spring Boot 就会在你的日志中包含相关的ID。

默认的相关ID是由 `traceId` 和 `spanId` [MDC](https://logback.qos.ch/manual/mdc.html) value 建立的。例如，如果Micrometer Tracing添加了一个MDC `traceId` 为 `803B448A0489F84084905D3093480352`，MDC `spanId` 为 `3425F23BB2432450`，日志输出将包括相关ID `[803B448A0489F84084905D3093480352-3425F23BB2432450]`。

如果你喜欢为你的相关 ID 使用不同的格式，你可以使用 `logging.pattern.correlation` 属性来定义一个。例如，下面将以 Spring Cloud Sleuth 以前使用的格式为 Logback 提供一个相关的 ID：
...
|      | 在上面的例子中， `logging.include-application-name` 被设置为 `false`，以避免 application name 在日志消息中被重复（ `logging.pattern.correlation` 已经包含它）。值得一提的是， `logging.pattern.correlation` 包含了一个尾部的空格，这样它就与默认情况下紧随其后的 logger 名称分开。 |
| ---- | ------------------------------------------------------------ |
|      |                                                              |
...
#### 8.4.1. 使用 Zipkin 的 OpenTelemetry

用OpenTelemetry跟踪并向Zipkin报告需要以下依赖：

- `io.micrometer:micrometer-tracing-bridge-otel` - 在OpenTelemetry中加入Micrometer Observation API。
- `io.opentelemetry:opentelemetry-exporter-zipkin` - 报告 traces 到 Zipkin。

使用 `management.zipkin.tracing.*` 配置属性来配置向Zipkin报告。

#### 8.4.2. 使用 Wavefront 的 OpenTelemetry

使用 OpenTelemetry 进行追踪并向 Wavefront 报告需要以下依赖：

- `io.micrometer:micrometer-tracing-bridge-otel` - 在OpenTelemetry中加入Micrometer Observation API。
- `io.micrometer:micrometer-tracing-reporter-wavefront` - 报告 trace 到 Wavefront。

使用 `management.wavefront.*` 配置属性来配置向 Wavefront 报告。

#### 8.4.3. 使用 OTLP 的 OpenTelemetry

使用 OpenTelemetry 进行追踪和使用 OTLP 进行报告需要以下依赖：

- `io.micrometer:micrometer-tracing-bridge-otel` - 在OpenTelemetry中加入Micrometer Observation API。
- `io.opentelemetry:opentelemetry-exporter-otlp` - 向可以接受 OTLP 的 collector 报告 trace。

使用 `management.otlp.tracing.*` 配置属性来配置使用 OTLP 的报告。

#### 8.4.4. 使用 Zipkin 的 OpenZipkin Brave

用 OpenZipkin Brave 追踪并向 Zipkin 报告需要以下依赖：

- `io.micrometer:micrometer-tracing-bridge-brave` - 在Brave中加入Micrometer Observation API 。
- `io.zipkin.reporter2:zipkin-reporter-brave` - 报告 trace 到 Zipkin。

|      | 如果你的项目没有使用Spring MVC或Spring WebFlux，也需要使用 `io.zipkin.reporter2:zipkin-sender-urlconnection` 依赖项。 |
| ---- | ------------------------------------------------------------ |
|      |                                                              |

使用 `management.zipkin.tracing.*` 配置属性来配置向 Zipkin 报告。

#### 8.4.5. 使用 Wavefront 的 OpenZipkin Brave

使用 OpenZipkin Brave 进行追踪并向 Wavefront 报告需要以下依赖：

- `io.micrometer:micrometer-tracing-bridge-brave` - 在Brave中加入 Micrometer Observation API。
- `io.micrometer:micrometer-tracing-reporter-wavefront` - 向 Wavefront 报告 trace 。

使用 `management.wavefront.*` 配置属性来配置向 Wavefront 报告。

### 8.5. 整合 Micrometer Observation

一个 `TracingAwareMeterObservationHandler` 被自动注册在 `ObservationRegistry` 上，它为每个完成的 observation 创建 span。
...
## 12. Cloud Foundry 的支持

Spring Boot 的actuator模块包括额外的支持，当你部署到兼容的 Cloud Foundry 实例时，该支持将被激活。 `/cloudfoundryapplication` 路径为所有 `@Endpoint` Bean提供了另一条安全路线。

扩展支持使 Cloud Foundry 管理 UI（例如你可以用来查看已部署的应用程序的 Web 应用程序）得到 Spring Boot 执行器信息的增强。 例如，应用程序状态页面可以包括完整的健康信息，而不是典型的 “running” 或 “stopped” 状态。

|      | 普通用户无法直接访问 `/cloudfoundryapplication` 路径。 要使用该端点，你必须在请求中传递一个有效的 UAA 令牌。 |
| ---- | ------------------------------------------------------------ |
|      |                                                              |

### 12.1. 禁用扩展的 Cloud Foundry Actuator 支持

如果你想完全禁用 `/cloudfoundryapplication` 端点，你可以在你的 `application.properties` 文件中添加以下设置。
...
如果你希望 Cloud Foundry 端点始终在 `/cloudfoundryapplication/*` 处可用，无论服务器的上下文路径如何，你需要在你的应用程序中明确配置。 该配置因使用的 Web 服务器不同而不同。 对于 Tomcat，你可以添加以下配置。
