...
线程池（`ThreadPoolExecutor`）是 Java 并发编程中至关重要的工具，位于 `java.util.concurrent` 包下。它通过复用一组预先创建的线程来执行任务，有效管理线程的生命周期，避免了频繁创建和销毁线程的开销，提高了系统性能、响应速度和资源利用率。

#### 为什么需要线程池？

*   **降低资源消耗**：创建和销毁线程是昂贵的操作（操作系统调用、内存分配）。线程池复用线程，显著减少此开销。
*   **提高响应速度**：任务到达时，无需等待线程创建，可直接由空闲线程执行，响应更快。
*   **提高线程的可管理性**：统一管理线程的创建、销毁、监控和调优，避免无限制创建线程导致系统资源耗尽（OOM）或 CPU 过度切换。
*   **控制并发度**：通过配置参数，可以精确控制同时运行的线程数量，防止系统过载。

#### 核心组件与参数

`ThreadPoolExecutor` 是 Java 线程池的核心实现，其构造函数有 7 个关键参数：
...
| 参数                               | 说明                                                         |
| :--------------------------------- | :----------------------------------------------------------- |
| **`corePoolSize` (核心线程数)**    | 线程池中**常驻**的线程数量。即使线程空闲，只要数量 <= `corePoolSize`，这些线程通常不会被销毁（除非设置了 `allowCoreThreadTimeOut`）。 |
| **`maximumPoolSize` (最大线程数)** | 线程池允许创建的**最大**线程数量。当任务队列已满且当前线程数 < `maximumPoolSize` 时，会创建新线程执行任务。 |
| **`keepAliveTime` (存活时间)**     | 当线程池中线程数量**超过 `corePoolSize`** 时，空闲线程在等待新任务时的**最长存活时间**。超过此时间且空闲的线程将被终止。 |
| **`unit` (时间单位)**              | `keepAliveTime` 的时间单位（如 `TimeUnit.SECONDS`）。        |
| **`workQueue` (任务队列)**         | 用于存放待执行任务的阻塞队列。当核心线程都在忙时，新提交的任务会进入此队列等待。**选择合适的队列至关重要**。 |
| **`threadFactory` (线程工厂)**     | 用于创建新线程的工厂。可以自定义线程的名称、优先级、是否为守护线程等。默认使用 `Executors.defaultThreadFactory()`。 |
| **`handler` (拒绝策略)**           | 当线程池已关闭或**无法接受新任务**（队列已满且线程数达到 `maximumPoolSize`）时，对新提交任务的处理策略。 |

#### 任务执行流程（核心逻辑）

1.  **提交任务**：调用 `execute(Runnable command)` 或 `submit(Callable<T> task)`。
2.  **检查核心线程**：如果当前运行的线程数 < `corePoolSize`，即使有空闲线程或队列不为空，也会**优先创建新线程**（直到达到 `corePoolSize`）来执行任务。
3.  **尝试入队**：如果当前线程数 >= `corePoolSize`，则尝试将任务**加入 `workQueue`**。
    *   **入队成功**：任务在队列中等待空闲线程处理。
    *   **入队失败**（队列已满）：
        *   如果当前线程数 < `maximumPoolSize`，则**创建新线程**（非核心线程）来执行此任务。
        *   如果当前线程数 >= `maximumPoolSize`，则**触发拒绝策略**。
4.  **线程复用与回收**：
    *   核心线程：默认情况下，核心线程会一直存活，即使空闲（可通过 `allowCoreThreadTimeOut(true)` 设置超时）。
    *   非核心线程：空闲时间超过 `keepAliveTime` 后会被回收。

> **关键点**：任务提交后，线程池的决策顺序是：**核心线程(正式合同工) -> 任务队列（人才库） -> 非核心线程（外包） -> 拒绝策略（人才库的外面）**。

#### 常见的 `workQueue` 类型

| 队列类型                    | 特点                                                         | 适用场景                                     |
| :-------------------------- | :----------------------------------------------------------- | :------------------------------------------- |
| **`ArrayBlockingQueue`**    | 有界阻塞队列，基于数组实现，容量固定。                       | 需要控制队列大小，防止资源耗尽。             |
| **`LinkedBlockingQueue`**   | 无界（或可选有界）阻塞队列，基于链表实现。**`Executors.newFixedThreadPool()` 和 `newSingleThreadExecutor()` 默认使用无界队列**。 | 吞吐量要求高，但需警惕内存溢出（OOME）。     |
| **`SynchronousQueue`**      | **不存储元素**的阻塞队列。每个插入操作必须等待另一个线程的移除操作（反之亦然），相当于直接将任务交给线程。**`Executors.newCachedThreadPool()` 默认使用**。 | 任务执行时间短，需要快速响应，线程复用率高。 |
| **`DelayQueue`**            | 元素只有在其延迟期满后才能被取出。                           | 定时任务调度。                               |
| **`PriorityBlockingQueue`** | 支持优先级排序的无界阻塞队列。                               | 需要按优先级处理任务。                       |

#### 常见的拒绝策略 (`RejectedExecutionHandler`)

| 策略                      | 行为                                                         | 说明                                                 |
| :------------------------ | :----------------------------------------------------------- | :--------------------------------------------------- |
| **`AbortPolicy` (默认)**  | 抛出 `RejectedExecutionException` 异常。                     | 最直接的方式，让调用者知道任务被拒绝。               |
| **`CallerRunsPolicy`**    | 由**提交任务的线程**（即调用 `execute` 的线程）**自己执行**该任务。 | 可以减缓新任务的提交速度，但可能影响调用线程的性能。 |
| **`DiscardPolicy`**       | **静默丢弃**被拒绝的任务，不抛异常。                         | 适用于允许丢失任务的场景。                           |
| **`DiscardOldestPolicy`** | 丢弃队列中**最老**的任务（队首），然后尝试重新提交被拒绝的任务。 | 尝试为新任务腾出空间，但仍可能失败。                 |

> 可以自定义 `RejectedExecutionHandler` 实现更复杂的逻辑（如记录日志、重试、降级等）。

####  `Executors` 工厂类（便捷创建）

`Executors` 类提供了创建常用线程池的静态方法：

*   **`Executors.newFixedThreadPool(int nThreads)`**:
    *   创建固定大小的线程池。
    *   `corePoolSize = maximumPoolSize = nThreads`。
    *   使用 **`LinkedBlockingQueue`（无界队列）**。
    *   **风险**：如果任务提交速度远大于处理速度，队列会无限增长，最终导致 `OutOfMemoryError`。
*   **`Executors.newCachedThreadPool()`**:
    *   创建一个可缓存的线程池。
    *   `corePoolSize = 0`, `maximumPoolSize = Integer.MAX_VALUE`。
    *   使用 **`SynchronousQueue`**。
    *   线程空闲 60 秒后会被回收。
    *   **风险**：极端情况下可能创建过多线程，耗尽系统资源。
*   **`Executors.newSingleThreadExecutor()`**:
    *   创建一个单线程的线程池。
    *   `corePoolSize = maximumPoolSize = 1`。
    *   使用 **`LinkedBlockingQueue`（无界队列）**。
    *   保证任务按顺序执行。
    *   **风险**：同 `newFixedThreadPool`，队列无界。
*   **`Executors.newScheduledThreadPool(int corePoolSize)`**:
    *   创建一个支持定时及周期性任务执行的线程池。
    *   用于替代 `Timer`。

> **重要提示**：阿里巴巴《Java开发手册》等规范**不建议**直接使用 `Executors` 工厂方法创建线程池，**强烈推荐通过 `new ThreadPoolExecutor(...)` 显式构造**，因为这样可以明确指定所有参数（尤其是有界队列），避免潜在的资源耗尽风险。

#### `Future` 和 `Callable`

*   `execute(Runnable)`：提交 `Runnable` 任务，无返回值。
*   `submit(Callable<T>)` 或 `submit(Runnable)`：提交任务并返回一个 `Future<T>` 对象。
    *   `Future` 可以用来：
        *   获取任务执行结果（`get()`，阻塞直到结果可用）。
        *   判断任务是否完成（`isDone()`）。
        *   取消任务（`cancel(boolean mayInterruptIfRunning)`）。
*   `Callable` 接口的 `call()` 方法可以返回结果并抛出异常，而 `Runnable` 的 `run()` 方法不能。

#### 最佳实践与注意事项

1.  **避免使用 `Executors` 的默认工厂方法**：优先使用 `new ThreadPoolExecutor(...)` 显式构造，**明确指定有界队列**（如 `ArrayBlockingQueue`），防止 OOM。
2.  **合理配置参数**：
    *   **CPU 密集型任务**：线程数 ≈ CPU 核心数 + 1（或 `Runtime.getRuntime().availableProcessors()`）。
    *   **IO 密集型任务**：线程数可以远大于 CPU 核心数（如 2 * CPU 核心数），因为线程在等待 IO 时会阻塞。
    *   根据业务场景、任务特性（CPU/IO）、系统资源（内存、CPU）和期望的吞吐量/延迟进行压测调优。
3.  **选择合适的 `workQueue`**：根据是否允许任务积压、队列大小限制、性能要求选择。
4.  **设置合适的拒绝策略**：根据业务需求选择或自定义，避免简单地抛异常或静默丢弃。
5.  **自定义 `ThreadFactory`**：为线程设置有意义的名称，便于问题排查和监控。
6.  **及时关闭线程池**：使用完毕后，务必调用 `shutdown()`（平滑关闭，不再接收新任务，等待已提交任务完成）或 `shutdownNow()`（立即关闭，尝试中断正在执行的任务，返回未开始的任务列表）来释放资源。通常在 `try-with-resources` 或 `finally` 块中执行。
7.  **监控**：监控线程池的关键指标，如：`getPoolSize()`, `getActiveCount()`, `getQueue().size()`, `getCompletedTaskCount()` 等，以便及时发现问题。
8.  **避免任务间依赖**：不要在任务中直接调用 `Future.get()` 等待另一个任务的结果，这可能导致死锁（尤其是在核心线程数较少时）。

#### 总结

Java 线程池是高效管理并发任务的强大工具。理解其核心参数、工作流程、队列选择、拒绝策略以及 `Executors` 工厂方法的潜在风险，是正确使用线程池的基础。**最佳实践是显式创建 `ThreadPoolExecutor` 实例，精心配置参数（尤其是使用有界队列），并做好监控和资源管理**，从而构建出稳定、高效、可维护的并发应用程序。
