...
响应头应里包含了 `text/event-stream`，说明使用的确实是SSE。并且，响应数据也确实是持续分块传输。
...
![僵尸进程查看](images/面试学习笔记/zombie-process-view.jpg)
...
![伙伴系统（Buddy System）内存管理](images/面试学习笔记/linux-buddy-system.png)
...
![伙伴系统内存浪费问题](images/面试学习笔记/buddy-system-memory-waste.png)
...
![虚拟内存作为进程访问主存的桥梁](images/面试学习笔记/virtual-memory.png)
...
在使用**虚拟内存**的操作系统中，当一个进程访问的虚拟内存页**不在物理内存中**时，就会触发一个 **缺页中断（或异常）**。这个中断是由**硬件（CPU 的内存管理单元 MMU）检测到**，然后交由**操作系统内核的缺页处理程序**来处理
...
> 你喜欢的up主你天天看，这是时间局部性；你喜欢的up主类似的up主你也可能爱看，这是空间局部性
...
是的，你的说法是正确的。RAID（Redundant Array of Independent Disks，独立磁盘冗余阵列）是一组将多个物理磁盘组合成一个或多个逻辑单元的技术，其主要目标包括：
...
- 原理：将数据分割成块，**并行写入多个磁盘**。
- 效果：
  - 读写速度显著提升（接近 N 倍，N 为磁盘数量）。
  - 但无冗余，任一磁盘损坏，数据全部丢失。

> ✅ 适用于对性能要求高、对数据安全性要求不高的场景（如视频编辑、临时缓存）。
...
> ✅ 适用于关键数据存储，如系统盘、数据库日志。

#### **条带化 + 奇偶校验（Striping with Parity）—— 平衡性能与安全**
- 常见级别：**RAID 5、RAID 6**
- 原理：
  - 数据条带化分布；
  - 奇偶校验信息分布存储，用于恢复损坏数据。
- 效果：
  - 读性能好；
  - 写性能因计算校验而略有开销；
  - 允许一个（RAID 5）或两个（RAID 6）磁盘故障。

> ✅ 适用于需要兼顾性能、容量和可靠性的场景，如文件服务器。

#### **混合 RAID（如 RAID 10）—— 高性能 + 高可靠**
- RAID 10 = RAID 1（镜像） + RAID 0（条带）
- 特点：
  - 高读写性能；
  - 强容错能力；
  - 存储利用率 50%。
...
| RAID 级别 | 性能提升 | 冗余 | 主要机制                |
| --------- | -------- | ---- | ----------------------- |
| RAID 0    | ⬆️⬆️⬆️      | ❌    | 条带化                  |
| RAID 1    | ⬆️（读）  | ✅    | 镜像                    |
| RAID 5    | ⬆️（读）  | ✅    | 条带化 + 分布式奇偶校验 |
| RAID 6    | ⬆️（读）  | ✅✅   | 条带化 + 双重奇偶校验   |
| RAID 10   | ⬆️⬆️       | ✅✅   | 镜像 + 条带化           |

#### 补充说明

除了 RAID，现代存储系统还使用其他技术进一步提升磁盘性能，例如：
- **SSD（固态硬盘）**：远高于机械硬盘的 I/O 性能；
- **NVMe 协议**：减少 I/O 延迟；
- **缓存（Cache）**：使用 RAM 或 SSD 作为磁盘缓存；
- **分布式存储（如 RAID 的演进：ZFS, RAID-Z）**

✅ 所以结论是：**RAID 是一种通过并行化和冗余设计来提高磁盘性能和可靠性的关键技术**，在服务器和存储系统中广泛应用。
...
> 类似于Linux/Unix的文件身份证号，出生就有，数量有限，通过身份证号可以找到这个文件的元数据
...
![文件inode信息](images/面试学习笔记/文件inode信息-DGIIRYlI.png)
...
| 排序算法 | Time complexity(时间复杂度)（平均） | 时间复杂度（最差）  | Time complexity (最好)  时间复杂度 （最好） | Space complexity(空间复杂度) | 排序方式 | 稳定性 |
| -------- | ----------------------------------- | ------------------- | ------------------------------------------- | ---------------------------- | -------- | ------ |
| 冒泡排序 | O(n^2) O（n^2）                     | O(n^2) O（n^2）     | O(n) O（n）                                 | O(1) （1）                   | 内部排序 | 稳定   |
| 选择排序 | O(n^2) O（n^2）                     | O(n^2) O（n^2）     | O(n^2) O（n^2）                             | O(1) （1）                   | 内部排序 | 不稳定 |
| 插入排序 | O(n^2) O（n^2）                     | O(n^2) O（n^2）     | O(n) O（n）                                 | O(1) （1）                   | 内部排序 | 稳定   |
| 希尔排序 | O(nlogn) O（nlogn）                 | O(n^2) O（n^2）     | O(nlogn) O（nlogn）                         | O(1) （1）                   | 内部排序 | 不稳定 |
| 归并排序 | O(nlogn) O（nlogn）                 | O(nlogn) O（nlogn） | O(nlogn) O（nlogn）                         | O(n) O（n）                  | 外部排序 | 稳定   |
| 快速排序 | O(nlogn) O（nlogn）                 | O(n^2) O（n^2）     | O(nlogn) O（nlogn）                         | O(logn) O（logn）            | 内部排序 | 不稳定 |
| 堆排序   | O(nlogn) O（nlogn）                 | O(nlogn) O（nlogn） | O(nlogn) O（nlogn）                         | O(1) （1）                   | 内部排序 | 不稳定 |
| 计数排序 | O(n+k) O（n+k）                     | O(n+k) O（n+k）     | O(n+k) O（n+k）                             | O(k) O（k）                  | 外部排序 | 稳定   |
| 桶排序   | O(n+k) O（n+k）                     | O(n^2) O（n^2）     | O(n+k) O（n+k）                             | O(n+k) O（n+k）              | 外部排序 | 稳定   |
| 基数排序 | O(n×k) O（n×k）                     | O(n×k) O（n×k）     | O(n×k) O（n×k）                             | O(n+k) O（n+k）              | 外部排序 | 稳定   |
...
6. 公司技术栈、业务方向、办公地点、薪资架构、福利待遇、加班情况、签约年限、新员工的培养计划、签订合同是和谁签、实习期是否全额工资、实习期长度、公司研发部门人员架构
