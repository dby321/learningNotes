...
```md
你是前端程序员专家，帮我给项目生成通用的全局基础布局，要求如下：
1) 文件名 layouts/BasicLayout.vue,并且在App.vue全局页面入口文件中引入
2) 移除main.css默认样式文件，以及对该文件的引用
3) BasicLayout整体布局为上中下布局，支持响应式，使用Ant Design Vue组件库的Layout实现
- 上方展示导航栏，独立创建GlobalHeader组件，左侧展示logo.png和网站标题，然后是菜单项，右侧展示登录用户的头像和昵称（暂时先用登录按钮替代），导航栏用Menu组件实现,支持通过配置设置菜单项
- 中间展示内容区域，根据路由切换页面
